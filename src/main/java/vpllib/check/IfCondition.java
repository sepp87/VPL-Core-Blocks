...
import vpllib.loop.TimeInterval;
...
public class IfCondition extends Block {
    ...
    public IfCondition(Workspace hostCanvas) {
        ...
        OPERATOR.getStyleClass().add("block-text");
    ...
    @Override
    public Block clone() {
        IfCondition block = new IfCondition(workspace);
        block.OPERATOR.setUserData(this.OPERATOR.getUserData());
        block.updateOperator();
        return block;
    }
}
